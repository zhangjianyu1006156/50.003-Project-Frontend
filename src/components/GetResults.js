...
const GetResults = async (setResults, filterSource, filter) => {
  ...
  if (filter !== undefined && filter !== null) {
    res = res.filter((item) => {
      const lowerprice = Math.min(item.kkdays_price, item.tripcom_price);
      if (filter.dest === "" && filter.budget === "") {
        return item;
      }
      if (
        item.name.toLowerCase().includes(filter.dest.toLowerCase()) ||
        parseFloat(lowerprice) <= parseFloat(filter.budget)
      ) {
        return item;
      }
      if (
